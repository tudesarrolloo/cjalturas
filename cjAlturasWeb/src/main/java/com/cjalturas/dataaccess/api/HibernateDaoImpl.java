...
public class HibernateDaoImpl<T, PK extends Serializable> implements Dao<T, PK> {
  ...
  /**
   * You cannot call EntityManager.persist() or EntityManager.remove() on a Detached object. {@link http ://openejb.apache.org/3.0/jpa-concepts.html}
   */
  ...
  public List<T> findPage(String sortColumnName, boolean sortAscending, int startRow, int maxResults) {
    ...
    if ((sortColumnName != null) && (sortColumnName.length() > 0)) {
      try {
        ...
        return sessionFactory.getCurrentSession().createQuery(queryString).setFirstResult(startRow).setMaxResults(maxResults).getResultList();
  ...
  public List<T> findByCriteria(String whereCondition) {
    ...
    try {
      ...
      List<T> entitiesList = sessionFactory.getCurrentSession().createQuery(queryString).getResultList();
